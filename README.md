...
- 图像理解建模：可直接在聊天框粘贴效果图，让视觉模型识别造型、比例、材质和构件层级，再生成建模方案或 Ruby 代码。
- 截图修正：可截取当前 SketchUp 视图，让视觉模型分析漂浮、错位、散件、门窗未贴墙等问题并生成修正建议或代码。
...
| Codex CLI 中转站 | 支持 | 支持视觉模型 | 默认 `https://www.hd1100.cc`，使用 Responses API，模型默认 `gpt-5.4`。 |
...
直接在聊天框里 `Ctrl+V` 粘贴效果图、参考图或截图即可。插件会显示缩略图，最多一次发送 3 张。

常用方式：

1. 粘贴效果图，输入“按这个图生成一个可编辑的 SketchUp 模型”，然后点发送或 `按图建模`。
2. 建模过程中切到合适视角，点 `截图分析`，让模型先指出当前问题。
3. 输入你的修正要求，点 `截图修正`，让模型根据当前截图和模型结构生成修正方案或 Ruby 代码。

DeepSeek 等非视觉模型会自动降级为文字上下文工作流；要直接看图，请使用 Gemini、Qwen-VL/MiMo-Omni、Codex CLI 中转站的视觉模型或其他支持图片输入的兼容模型。
